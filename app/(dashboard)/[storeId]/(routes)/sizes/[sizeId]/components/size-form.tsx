"use client";
///the form that will be created or activated when a user wants to create a new size or update an existing one
...
const formSchema = z.object({
	///the name of size schema ppt then z.type.minimum character expected
	name: z.string().min(1),
	///the image URL of the size schema
...
interface SizeFormProps {
	///we use the Size as the inital data of the prop or null if billbpard is not found
...
export const SizeForm: React.FC<SizeFormProps> = ({ initialData }) => {
	...
	///we create new variables that will help check if there is initial data (ie size aready exists)to dynamically edit the props  of the size form else we create a new size
	...
	const form = useForm<SizeFormValues>({
		...
		defaultValues: initialData || {
			name: "",
			value: "",
		}, ///giving it default values of the initial data which is the prop we are passing in and setting it automatically to empty strings if the initial Data does not exist which is a size schema
	});
	...
	const onSubmit = async (data: SizeFormValues) => {
		try {
			...
			router.refresh();
			///After we submit our page we want to redirect back to the home sizes page
	...
	const onDelete = async () => {
		try {
			...
			router.push(`/${params.storeId}/sizes`); ///which is the root route
	...
	return (
		...
		<>
			...
			<div className='flex items-center justify-between'>
				...
				{/* We put a conditional to determine if that delete button will show i this size form like the settings form or not*/}
				{initialData && (
					<Button
						disabled={loading}
						variant='destructive'
